...
def test_scoring_produces_required_output_columns(tmp_path):
    ...
    assert {"trend_data_status", "trend_refresh_mode", "trend_cache_age_hours", "trend_provider", "trend_source_detail", "operating_mode", "scoring_profile", "confidence_score", "ml_model_status"}.issubset(reloaded.columns)
...
def test_verify_outputs_accepts_market_fundamental_not_used_trends(tmp_path, monkeypatch):
    row = {column: 1 for column in verifier.REQUIRED_COLUMNS}
    row.update({
        "sector": "Example",
        "ticker": "EX",
        "trend_data_status": "not_used",
        "trend_provider": "disabled_by_mode",
        "trend_source_detail": "disabled",
        "trend_refresh_mode": "auto",
        "trend_cache_age_hours": pd.NA,
        "operating_mode": "market_fundamental",
        "scoring_profile": "market_fundamental_only",
        "recommendation": "Watch",
        "short_explanation": "Market/fundamental-only mode",
        "total_score": 60,
    ...
    monkeypatch.setattr(verifier, "HTML_REPORT_PATH", html_path)
    monkeypatch.setattr(verifier, "DASHBOARD_PATH", dashboard_path)
    monkeypatch.setattr(verifier, "MODEL_PATH", tmp_path / "missing.pkl")
